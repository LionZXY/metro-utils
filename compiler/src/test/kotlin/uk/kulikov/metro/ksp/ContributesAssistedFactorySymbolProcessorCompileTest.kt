...
class MultipleRoundProcessingTest {
    ...
    fun `should handle multiple KSP rounds correctly`() {
        ...
        val generatedComponent = runCatching {
            result.classLoader.loadClass("uk.kulikov.metro.sample.SampleComponentGenerated")
        }
        ...
        val generatedFactory = kotlin.runCatching {
            result.classLoader.loadClass("uk.kulikov.metro.sample.DefaultSampleComponent_AssistedFactory")
        }
        assertNotNull(
    ...
    fun `should defer processing when types are not yet available`() {
                    ...
                    override fun create(environment: SymbolProcessorEnvironment): SymbolProcessor {
                        return object : SymbolProcessor {
                            ...
                            override fun process(resolver: Resolver): List<KSAnnotated> {
                                if (!generated) {
                                    FileSpec.builder("uk.kulikov.metro.sample", "SampleComponentGenerated1")
                                        .addType(
                    ...
                    override fun create(environment: SymbolProcessorEnvironment): SymbolProcessor {
                        return object : SymbolProcessor {
                            ...
                            override fun process(resolver: Resolver): List<KSAnnotated> {
                                ...
                                if (roundCount == 2) {
                                    FileSpec.builder("uk.kulikov.metro.sample", "SampleComponentGenerated2")
                                        .addType(
        ...
        listOf(
            ...
            assertNotNull(
                result.classLoader.loadClass("uk.kulikov.metro.sample.$className"),
                    ...
                    override fun create(environment: SymbolProcessorEnvironment): SymbolProcessor {
                        return object : SymbolProcessor {
                            ...
                            private fun generateClass(name: String) {
                                FileSpec.builder("uk.kulikov.metro.sample", name)
    ...
    fun `should fail compilation with proper errors when types are unresolved`() {
        ...
        assertThat(
            result.classLoader.runCatching {
                loadClass("uk.kulikov.metro.sample.InvalidComponent_AssistedFactory")
            }.isFailure
        ).isTrue()
    }
    ...
    fun `should fail compilation when only some types are unresolved`() {
        ...
        assertThat(
            result.classLoader.runCatching {
                loadClass("uk.kulikov.metro.sample.PartiallyInvalidComponent_AssistedFactory")
            }.isFailure
        ).isTrue()
    }
